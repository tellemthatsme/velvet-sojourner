...
class GitRepoDownloader:
    ...
    def download_selective(self, task: DownloadTask) -> bool:
        ...
        github_client = GitHubAPIClient(self.token)
        target_dir = os.path.join(task.output_path, task.repo)
        os.makedirs(target_dir, exist_ok=True)
        
        total = len(task.selected_paths)
        downloaded = 0
        ...
        try:
            for path in task.selected_paths:
                ...
                self.update_progress(task.id, progress=(downloaded / total) * 100, 
                                   current_file=f"Downloading {path}...")
                
                local_path = os.path.join(target_dir, path)
                
                # Check if it's a file or directory in our tree (we don't have types here yet, 
                # but download_file handles non-files by returning False)
                success, result = github_client.download_file(task.owner, task.repo, path, local_path)
                
                if not success:
                    # If it's a directory, just ensure it exists locally
                    # Note: We expect the recursive tree to have provided all files inside
                    if "is not a file" in str(result):
                        os.makedirs(local_path, exist_ok=True)
                    else:
                        # Log error but continue with other files? 
                        # For now, let's just log it in task.current_file
                        print(f"Error downloading {path}: {result}")
