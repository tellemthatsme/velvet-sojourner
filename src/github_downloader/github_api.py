...
class GitHubRateLimitHandler:
    ...
    def check_rate_limit(
        self, headers: Dict, is_authenticated: bool = False
    ) -> RateLimitInfo:
...
class GitHubAPIClient:
    ...
    def __init__(
        ...
        self._cache = {}
        self._cache_ttl = 300
        self._cache_max = 100  # Evict oldest entries beyond this size
    ...
    def _make_request(
        ...
        rate_info = self.rate_handler.check_rate_limit(
            response.headers, self.authenticated
        )
    ...
    def get_repo(self, owner: str, repo: str) -> Optional[Dict]:
        ...
        response = self._make_request(
            "GET", f"{self.BASE_URL}/repos/{owner}/{repo}", use_cache=True
        )
        if response.status_code == 200:
            return response.json()
        return None
    ...
    def get_repo_contents(
        self, owner: str, repo: str, path: str = "", progress_callback: Callable = None
    ) -> List[Dict]:
    ...
    def get_repo_tree(
        self, owner: str, repo: str, recursive: bool = False
    ) -> Optional[Dict]:
        """Get repository tree"""
    ...
    def get_releases(self, owner: str, repo: str, per_page: int = 100) -> List[Dict]:
        """Get repository releases with assets"""
        releases = []
        page = 1
        while page <= 10:
            ...
            for release in data:
                if release.get("assets"):
                    release["_assets_details"] = self._get_release_assets(
    ...
    def _get_release_assets(self, owner: str, repo: str, release_id: int) -> List[Dict]:
    ...
    def get_wiki(self, owner: str, repo: str) -> Optional[str]:
        """Get repository wiki content (if available)"""
        url = f"{self.BASE_URL}/repos/{owner}/{repo}/wiki"
        response = self._make_request("GET", url)
        if response.status_code == 200:
            return response.text
        return None

    def get_gists(self, username: str, per_page: int = 100) -> List[Dict]:
        """Get user's gists"""
        gists = []
        page = 1
        while page <= 10:
    ...
    def get_starred(self, username: str, per_page: int = 100) -> List[Dict]:
