...
class IncrementalDownloader:
    ...
    def _incremental_sync(self, repo_url: str, local_path: str,
        ...
        try:
            ...
            remotes = subprocess.run(['git', 'remote'], cwd=local_path, capture_output=True, text=True, env=env).stdout.split()
            if 'origin' in remotes:
                subprocess.run(['git', 'remote', 'set-url', 'origin', remote_url], cwd=local_path, capture_output=True, env=env)
            else:
                subprocess.run(['git', 'remote', 'add', 'origin', remote_url], cwd=local_path, capture_output=True, env=env)
        except Exception as e:
            errors.append(f"Remote configuration failed: {e}")
        ...
        if result.returncode != 0:
            ...
            if "authentication failed" in err_msg:
                errors.append("GitHub authentication failed. Please check your token.")
            elif "could not resolve host" in err_msg:
                errors.append("Network error: Could not resolve GitHub host.")
            else:
        ...
        # Detect Default Branch Robusted
        remote_head = "origin/main" # Default fallback
        try:
            # More reliable way to get the remote default branch
            show_res = subprocess.run(
                ['git', 'remote', 'show', 'origin'],
                cwd=local_path, capture_output=True, text=True, env=env
            )
            for line in show_res.stdout.split('\n'):
                if "HEAD branch:" in line:
                    remote_head = f"origin/{line.split(':')[-1].strip()}"
                    break
        except:
            # Fallback to broad detection
