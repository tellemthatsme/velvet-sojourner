...
class GitRepoDownloader:
    ...
    def download_selective(self, task: DownloadTask) -> bool:
        """Download only specific files and folders"""
        if not task.selected_paths:
            task.error = "No files or folders selected"
            task.status = DownloadStatus.FAILED
            return False
            
        repo_url = f"https://api.github.com/repos/{task.owner}/{task.repo}/contents"
        headers = {'Accept': 'application/vnd.github.v3+json'}
        if self.token:
            headers['Authorization'] = f'token {self.token}'
            
        target_base = os.path.join(task.output_path, task.repo)
        os.makedirs(target_base, exist_ok=True)
        
        total_items = len(task.selected_paths)
        downloaded_count = 0
        
        try:
            for path in task.selected_paths:
                self.update_progress(task.id, progress=(downloaded_count / total_items) * 100, current_file=f"Downloading {path}...")
                
                # Download item (recursive if directory)
                success = self._download_recursive(task.owner, task.repo, path, target_base, headers)
                if not success:
                    # Continue with others but mark as error later?
                    pass
    ...
    def _download_recursive(self, owner, repo, path, target_base, headers):
        """Helper to download file or directory recursively"""
        ...
        if isinstance(data, list): # It's a directory
            os.makedirs(os.path.join(target_base, path), exist_ok=True)
            for item in data:
                self._download_recursive(owner, repo, item['path'], target_base, headers)
        else: # It's a file
            download_url = data.get('download_url')
            if download_url:
                local_path = os.path.join(target_base, path)
    ...
    def start_download(self, task_id: str) -> bool:
        ...
        elif task.method == DownloadMethod.SELECTIVE:
            return self.download_selective(task)
